...
*Дополнительно:*
• ❌ Без кнопок и элементов, имитирующих интерфейс
• ✅ Корректная работа в браузерах: IE 10+, Firefox 14+, Safari 5+, Chrome 18+
• 📏 Сейф-зона: *по 60 пикселей сверху и снизу*
"""
    },
...
• 🖌 Креатив должен иметь видимую границу, отличную по цвету от фона
...
• 💾 Максимальный вес файла: *до 150 КБ* (архив)
• 📁 Формат: *ZIP архив*
• 🖌 Креатив должен иметь видимую границу

*Содержимое архива:*
• index.html (единственный HTML-файл)
• .png, .jpg, .jpeg, .gif, .css, .js, .svg (до 600 КБ)
...
• ✅ Все файлы в одной папке (без вложенных папок)
• ✅ Обертка <body> в `<div id="Stage">...</div>`
• ✅ Запрещены теги `<a>`
• ✅ Сторонний параметр для защиты от кэша: `![random]`

*Разрешённые внешние библиотеки:*
• createjs-*.min.js (Adobe Edge Animate)
• runtime.js (Swiffy)
• Enabler.js (Google Web Designer)
• jquery-*.min.js (jQuery)
• TweenMax.min.js (GreenSock)
"""
    },
...
• 📏 *60 пикселей сверху и снизу*
   (вверху: плашка рекламы и erid, внизу: плашка пропуска)
"""
    },
    
    # ========== TCL ==========
    "tcl_homepage_video": {
        "name": "📺 TCL - Homepage Ad slot (Video) / TCL Channel",
        "position": "Пометка располагается в *правом верхнем углу*",
        "requirements": """
📺 *Технические требования TCL - Homepage Ad slot (Video):*

• ⏱ Хронометраж: *не более 60 секунд*
• 💾 Максимальный объем: *не ограничен*
...
• 📁 Формат: *JPG / JPEG*
• 💾 Максимальный вес: *до 500 КБ*
• 📐 Размер: *372×210*
"""
    },
    "tcl_homepage_banner": {
        "name": "🖼 TCL - Homepage banner Adslot",
        "position": "Пометка располагается в *левом верхнем углу*",
        "requirements": """
🖼 *Технические требования TCL - Homepage banner Adslot:*
...
"""
    ...
    # ========== Xiaomi ==========
    "xiaomi_patchwall_banner": {
        "name": "🖼 Xiaomi - PatchWall (баннер)",
        "position": "Пометка располагается в *левом верхнем углу*",
        "requirements": """
🖼 *Технические требования Xiaomi - PatchWall (баннер):*

• 📐 Размер баннера: *640×360*
• 📁 Формат: *JPG*
• 💾 Вес: *до 300 КБ*
...
"""
    ...
    "xiaomi_patchwall_video": {
        "name": "📹 Xiaomi - PatchWall (видео)",
        "position": "Пометка располагается в *правом верхнем углу*",
        "requirements": """
📹 *Технические требования Xiaomi - PatchWall (видео):*

• 🎬 Качество: *не ниже 720p*
• 📐 Размер: *1920×1080*
...
"""
    ...
    # ========== Haier ==========
    "haier_banner": {
        "name": "🖼 Haier - Баннер на главной странице Haier evo tv",
        "position": "Пометка располагается в *левом верхнем углу*",
        "requirements": """
🖼 *Технические требования Haier:*

• 💾 Максимальный вес: *до 5 МБ*
• 📐 Рекомендуемое разрешение: *2136×896*
• 📁 Формат: *JPG / JPEG / PNG*
"""
    }
}
...
# Клавиатура для Xiaomi
xiaomi_keyboard = InlineKeyboardMarkup(inline_keyboard=[
    [
        InlineKeyboardButton(text="🖼 Баннер", callback_data="xiaomi_patchwall_banner"),
        InlineKeyboardButton(text="📹 Видео", callback_data="xiaomi_patchwall_video")
    ],
    [InlineKeyboardButton(text="◀️ Назад", callback_data="back_to_main")]
])
...
async def show_marketing(message: types.Message):
    ...
    await message.answer("Выбери формат для технических требований:", reply_markup=main_keyboard)
...
async def show_xiaomi_menu(message: types.Message):
    await message.answer("Выбери тип креатива для Xiaomi:", reply_markup=xiaomi_keyboard)
...
async def handle_callback(callback: types.CallbackQuery):
    ...
    # Xiaomi
    elif callback.data == "xiaomi_patchwall_banner":
        data = TECH_REQUIREMENTS["xiaomi_patchwall_banner"]
        await callback.message.edit_text(
            f"{data['requirements']}\n\n📍 *Расположение пометки:* {data['position']}",
            parse_mode="Markdown"
        )
    elif callback.data == "xiaomi_patchwall_video":
        data = TECH_REQUIREMENTS["xiaomi_patchwall_video"]
        await callback.message.edit_text(
            f"{data['requirements']}\n\n📍 *Расположение пометки:* {data['position']}",
            parse_mode="Markdown"
        )
